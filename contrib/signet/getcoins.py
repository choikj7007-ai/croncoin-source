...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing croncoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='croncoin-cli', help='croncoin-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='CronCoin address to which the faucet should send')
...
parser.add_argument('croncoin_cli_args', nargs='*', help='Arguments to pass on to croncoin-cli (default: -signet)')

args = parser.parse_args()

if args.croncoin_cli_args == []:
    args.croncoin_cli_args = ['-signet']


def croncoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.croncoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = croncoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = croncoin_cli(['getnewaddress', 'faucet', 'bech32'])
