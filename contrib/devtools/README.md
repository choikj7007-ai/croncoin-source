...
gen-croncoin-conf.sh
===================

Generates a croncoin.conf file in `share/examples/` by parsing the output from `croncoind --help`. This script is run during the
release process to include a croncoin.conf with the release binaries and can also be run by users to generate a file locally.
...
```bash
BUILDDIR=$PWD/my-build-dir contrib/devtools/gen-croncoin-conf.sh
```
