...
be used in CronCoin Core with the `-asmap` option.
...
to binary before being used with CronCoin Core.
...
the `croncoin-cli getnodeaddresses` command.
...
```
croncoin-cli getnodeaddresses 0 > addrs.json
```
