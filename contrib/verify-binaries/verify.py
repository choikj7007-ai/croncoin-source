...
"""Script for verifying CronCoin Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from croncoincore.org and croncoin.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/croncoin-core/guix.sigs/tree/main/builder-keys
...
HOST1 = "https://croncoincore.org"
HOST2 = "https://croncoin.org"
VERSIONPREFIX = "croncoin-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, list[SigData], list[SigData], list[SigData], list[SigData]
    ...
    # which pubkeys within the CronCoin community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"croncoin_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by croncoincore.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the croncoin release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes croncoin.org lags behind croncoincore.org.)')
    )
