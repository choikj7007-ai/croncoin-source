# Bootstrappable CronCoin Core Builds

This directory contains the files necessary to perform bootstrappable CronCoin
Core builds.
...
To build CronCoin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      CronCoin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/croncoin-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/croncoin-core/guix.sigs).
