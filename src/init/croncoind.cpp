...
namespace init {
namespace {
const char* EXE_NAME = "croncoind";

class CroncoindInit : public interfaces::Init
{
public:
    CroncoindInit(NodeContext& node) : m_node(node)
    {
        InitContext(m_node);
        m_node.init = this;
    }
...
{
    return std::make_unique<init::CroncoindInit>(node);
}
} // namespace interfaces
