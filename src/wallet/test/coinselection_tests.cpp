...
BOOST_AUTO_TEST_CASE(bnb_test)
{
    ...
    for (int feerate : feerates) {
        ...
        TestBnBFail("Empty UTXO pool", utxo_pool, /*selection_target=*/1 * COIN);

        AddCoins(utxo_pool, {1 * COIN, 3 * COIN, 5 * COIN}, cs_params);

        // Simple success cases
        TestBnBSuccess("Select smallest UTXO", utxo_pool, /*selection_target=*/1 * COIN, /*expected_input_amounts=*/{1 * COIN}, cs_params);
        TestBnBSuccess("Select middle UTXO", utxo_pool, /*selection_target=*/3 * COIN, /*expected_input_amounts=*/{3 * COIN}, cs_params);
        TestBnBSuccess("Select biggest UTXO", utxo_pool, /*selection_target=*/5 * COIN, /*expected_input_amounts=*/{5 * COIN}, cs_params);
        TestBnBSuccess("Select two UTXOs", utxo_pool, /*selection_target=*/4 * COIN, /*expected_input_amounts=*/{1 * COIN, 3 * COIN}, cs_params);
        TestBnBSuccess("Select all UTXOs", utxo_pool, /*selection_target=*/9 * COIN, /*expected_input_amounts=*/{1 * COIN, 3 * COIN, 5 * COIN}, cs_params);

        // BnB finds changeless solution while overshooting by up to cost_of_change
        TestBnBSuccess("Select upper bound", utxo_pool, /*selection_target=*/4 * COIN - default_cs_params.m_cost_of_change, /*expected_input_amounts=*/{1 * COIN, 3 * COIN}, cs_params);

        // BnB fails to find changeless solution when overshooting by cost_of_change + 1 cro
        TestBnBFail("Overshoot upper bound", utxo_pool, /*selection_target=*/4 * COIN - default_cs_params.m_cost_of_change - 1);

        TestBnBSuccess("Select max weight", utxo_pool, /*selection_target=*/4 * COIN, /*expected_input_amounts=*/{1 * COIN, 3 * COIN}, cs_params, /*custom_spending_vsize=*/P2WPKH_INPUT_VSIZE, /*max_selection_weight=*/4 * 2 * P2WPKH_INPUT_VSIZE);

        TestBnBFail("Exceed max weight", utxo_pool, /*selection_target=*/4 * COIN, /*max_selection_weight=*/4 * 2 * P2WPKH_INPUT_VSIZE - 1, /*expect_max_weight_exceeded=*/true);

        // Simple cases without BnB solution
        TestBnBFail("Smallest combination too big", utxo_pool, /*selection_target=*/COIN / 2);
        TestBnBFail("No UTXO combination in target window", utxo_pool, /*selection_target=*/7 * COIN);
        TestBnBFail("Select more than available", utxo_pool, /*selection_target=*/10 * COIN);

        // Test skipping of equivalent input sets
        std::vector<OutputGroup> clone_pool;
        AddCoins(clone_pool, {2 * COIN, 7 * COIN, 7 * COIN}, cs_params);
        AddDuplicateCoins(clone_pool, 50'000, 5 * COIN, cs_params);
        TestBnBSuccess("Skip equivalent input sets", clone_pool, /*selection_target=*/16 * COIN, /*expected_input_amounts=*/{2 * COIN, 7 * COIN, 7 * COIN}, cs_params);
        ...
        // Use a large base amount (100 COIN) so cost_of_change is negligible relative to base
        static constexpr CAmount DOPPEL_BASE = 100 * COIN;
        for (int i = 0; i < 17; ++i) {
            if (i < 8) {
                // The eight smallest UTXOs can be combined to create expected_result
                doppelgangers.push_back(DOPPEL_BASE + i);
                ...
                doppelgangers.push_back(DOPPEL_BASE + default_cs_params.m_cost_of_change + i);
            }
        }
        ...
        TestBnBSuccess("Combine smallest 8 of 17 unique UTXOs", doppelganger_pool, /*selection_target=*/8 * DOPPEL_BASE, /*expected_input_amounts=*/expected_inputs, cs_params);
        ...
        AddCoins(doppelganger_pool, {DOPPEL_BASE + default_cs_params.m_cost_of_change + 17}, cs_params);
        TestBnBFail("Exhaust looking for smallest 8 of 18 unique UTXOs", doppelganger_pool, /*selection_target=*/8 * DOPPEL_BASE);
    }
}
...
BOOST_AUTO_TEST_CASE(bnb_feerate_sensitivity_test)
{
    ...
    AddCoins(low_feerate_pool, {2 * COIN, 3 * COIN, 5 * COIN, 10 * COIN});
    TestBnBSuccess("Select many inputs at low feerates", low_feerate_pool, /*selection_target=*/10 * COIN, /*expected_input_amounts=*/{2 * COIN, 3 * COIN, 5 * COIN});
    ...
    AddCoins(high_feerate_pool, {2 * COIN, 3 * COIN, 5 * COIN, 10 * COIN}, high_feerate_params);
    TestBnBSuccess("Select one input at high feerates", high_feerate_pool, /*selection_target=*/10 * COIN, /*expected_input_amounts=*/{10 * COIN}, high_feerate_params);

    // Add heavy inputs {6, 7} to existing {2, 3, 5, 10}
    low_feerate_pool.push_back(MakeCoin(6 * COIN, true, default_cs_params, /*custom_spending_vsize=*/500));
    low_feerate_pool.push_back(MakeCoin(7 * COIN, true, default_cs_params, /*custom_spending_vsize=*/500));
    TestBnBSuccess("Prefer two heavy inputs over two light inputs at low feerates", low_feerate_pool, /*selection_target=*/13 * COIN, /*expected_input_amounts=*/{6 * COIN, 7 * COIN}, default_cs_params, /*custom_spending_vsize=*/500);

    high_feerate_pool.push_back(MakeCoin(6 * COIN, true, high_feerate_params, /*custom_spending_vsize=*/500));
    high_feerate_pool.push_back(MakeCoin(7 * COIN, true, high_feerate_params, /*custom_spending_vsize=*/500));
    TestBnBSuccess("Prefer two light inputs over two heavy inputs at high feerates", high_feerate_pool, /*selection_target=*/13 * COIN, /*expected_input_amounts=*/{3 * COIN, 10 * COIN}, high_feerate_params);
}
