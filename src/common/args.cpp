...
const char * const CRONCOIN_CONF_FILENAME = "croncoin.conf";
const char * const CRONCOIN_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //croncoin-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", CRONCOIN_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\CronCoin
    //   new: C:\Users\Username\AppData\Local\CronCoin
    // macOS: ~/Library/Application Support/CronCoin
    // Unix-like: ~/.croncoin
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "CronCoin";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "CronCoin";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/CronCoin";
#else
    // Unix-like
    return pathRet / ".croncoin";
#endif
#endif
}
