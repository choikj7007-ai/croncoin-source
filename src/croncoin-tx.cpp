...
static void SetupCroncoinTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " croncoin-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The croncoin-tx tool is used for creating and modifying croncoin transactions.\n\n"
                "croncoin-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded croncoin transaction, or with \"-create [commands]\" to create a hex-encoded croncoin transaction.\n"
                "\n"
                "Usage: croncoin-tx [options] <hex-tx> [commands]\n"
                "or:    croncoin-tx [options] -create [commands]\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded croncoin transaction
