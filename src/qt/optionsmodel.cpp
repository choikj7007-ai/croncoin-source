...
/** Call node.updateRwSetting() with CronCoin 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because CronCoin 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/croncoin/croncoin/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because croncoin 22.x will not try to read these.
...
//! Convert enabled/size values to croncoin -prune setting.
...
//! Get pruning enabled value to show in GUI from croncoin -prune setting.
...
//! Get pruning size value to show in GUI from croncoin -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayCronCoinUnit")) {
        settings.setValue("DisplayCronCoinUnit", QVariant::fromValue(CronCoinUnit::BTC));
    }
    QVariant unit = settings.value("DisplayCronCoinUnit");
    if (unit.canConvert<CronCoinUnit>()) {
        m_display_croncoin_unit = unit.value<CronCoinUnit>();
    } else {
        m_display_croncoin_unit = CronCoinUnit::BTC;
        settings.setValue("DisplayCronCoinUnit", QVariant::fromValue(m_display_croncoin_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if croncoin.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_croncoin_unit);
...
{
    if (new_unit.isNull() || new_unit.value<CronCoinUnit>() == m_display_croncoin_unit) return;
    m_display_croncoin_unit = new_unit.value<CronCoinUnit>();
    QSettings settings;
    settings.setValue("DisplayCronCoinUnit", QVariant::fromValue(m_display_croncoin_unit));
    Q_EMIT displayUnitChanged(m_display_croncoin_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/croncoin/croncoin/pull/8273
    ...
    // (https://github.com/croncoin-core/gui/issues/567).
