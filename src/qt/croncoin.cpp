...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaType<CronCoinUnit>("CronCoinUnit");
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. croncoin_de.qm (shortcut "de" needs to be defined in croncoin.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. croncoin_de_DE.qm (shortcut "de_DE" needs to be defined in croncoin.qrc)
...
static const char* qt_argv = "croncoin-qt";

CronCoinApplication::CronCoinApplication()
...
void CronCoinApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the CronCoinApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", CronCoinGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void CronCoinApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool CronCoinApplication::createOptionsModel(bool resetSettings)
{
...
void CronCoinApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new CronCoinGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &CronCoinGUI::quitRequested, this, &CronCoinApplication::requestShutdown);
...
void CronCoinApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void CronCoinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool CronCoinApplication::baseInitialize()
{
    return node().baseInitialize();
}

void CronCoinApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &CronCoinApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &CronCoinApplication::handleRunawayException);
    connect(this, &CronCoinApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &CronCoinApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void CronCoinApplication::parameterSetup()
{
...
void CronCoinApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void CronCoinApplication::requestInitialize()
{
...
void CronCoinApplication::requestShutdown()
{
...
void CronCoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    // croncoin: URIs or payment requests:
    if (paymentServer) {
        connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &CronCoinGUI::handlePaymentRequest);
        connect(window, &CronCoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void CronCoinApplication::handleRunawayException(const QString &message)
{
...
void CronCoinApplication::handleNonFatalException(const QString& message)
{
...
WId CronCoinApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool CronCoinApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", CronCoinGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(croncoin);
    Q_INIT_RESOURCE(croncoin_locale);
    ...
    for (int i = 1; i < argc; i++) {
...
#ifdef ENABLE_WALLET
        if (arg.startsWith(CRONCOIN_IPC_PREFIX, Qt::CaseInsensitive)) {
            invalid_token &= false;
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see croncoin-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, CLIENT_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see croncoin-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse croncoin.conf, determine network, switch to network specific
    ...
    // croncoin: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
