...
#include <croncoin-build-config.h> // IWYU pragma: keep

#include <qt/croncoinqgui.h>

#include <qt/croncoinunits.h>
 ...
 *
 * Ref: https://github.com/croncoin/croncoin/pull/1026
 */
static constexpr int64_t MAX_BLOCK_TIME_GAP = 90 * 60;

const std::string CronCoinGUI::DEFAULT_UIPLATFORM =
...
CronCoinGUI::CronCoinGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &CronCoinGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &CronCoinGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &CronCoinGUI::showModalOverlay);
...
void CronCoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a CronCoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and croncoin: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &CronCoinGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &CronCoinGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &CronCoinGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your CronCoin addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified CronCoin addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed CronCoin Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed CronCoin Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a croncoin: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible CronCoin command-line options").arg(CLIENT_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &CronCoinGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &CronCoinGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &CronCoinGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &CronCoinGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &CronCoinGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &CronCoinGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [path, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &CronCoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &CronCoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &CronCoinGUI::createWallet);
        ...
        connect(m_migrate_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [wallet_name, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, wallet_name] {
                    ...
                    connect(activity, &MigrateWalletActivity::migrated, this, &CronCoinGUI::setCurrentWallet);
        ...
        connect(m_mask_values_action, &QAction::toggled, this, &CronCoinGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &CronCoinGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &CronCoinGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &CronCoinGUI::showDebugWindow);
}

void CronCoinGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void CronCoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &CronCoinGUI::setCurrentWalletBySelectorIndex);
...
void CronCoinGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &CronCoinGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &CronCoinGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &CronCoinGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &CronCoinGUI::showProgress);
...
#ifdef ENABLE_WALLET
void CronCoinGUI::enableHistoryAction(bool privacy)
{
...
void CronCoinGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &CronCoinGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &CronCoinGUI::removeWallet);
...
WalletController* CronCoinGUI::getWalletController()
{
    return m_wallet_controller;
}

void CronCoinGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &CronCoinGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &CronCoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &CronCoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &CronCoinGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &CronCoinGUI::incomingTransaction);
    connect(this, &CronCoinGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void CronCoinGUI::removeWallet(WalletModel* walletModel)
{
...
void CronCoinGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void CronCoinGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void CronCoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void CronCoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void CronCoinGUI::createTrayIcon()
{
...
void CronCoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &CronCoinGUI::toggleHidden);
...
void CronCoinGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void CronCoinGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void CronCoinGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void CronCoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void CronCoinGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void CronCoinGUI::openClicked()
{
...
void CronCoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void CronCoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void CronCoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void CronCoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void CronCoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void CronCoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void CronCoinGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void CronCoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to CronCoin network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void CronCoinGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void CronCoinGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void CronCoinGUI::updateHeadersSyncProgressLabel()
{
...
void CronCoinGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void CronCoinGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &CronCoinGUI::quitRequested);
...
void CronCoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void CronCoinGUI::createWallet()
{
#ifdef ENABLE_WALLET
    auto activity = new CreateWalletActivity(getWalletController(), this);
    connect(activity, &CreateWalletActivity::created, this, &CronCoinGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void CronCoinGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void CronCoinGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &CronCoinGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &CronCoinGUI::show);
                e->ignore();
            }
        }
    }
...
void CronCoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void CronCoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void CronCoinGUI::incomingTransaction(const QString& date, CronCoinUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(CronCoinUnits::formatWithUnit(unit, amount, true));
...
void CronCoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void CronCoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool CronCoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool CronCoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void CronCoinGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void CronCoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void CronCoinGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void CronCoinGUI::updateProxyIcon()
{
    std::string ip_port;
...
void CronCoinGUI::updateWindowTitle()
{
...
void CronCoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void CronCoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void CronCoinGUI::detectShutdown()
{
...
void CronCoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void CronCoinGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(CronCoinGUI* gui, const bilingual_str& message, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = CronCoinGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void CronCoinGUI::subscribeToCoreSignals()
{
...
void CronCoinGUI::unsubscribeFromCoreSignals()
{
...
bool CronCoinGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<CronCoinUnit> units = CronCoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const CronCoinUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, CronCoinUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const CronCoinUnit u : CronCoinUnits::availableUnits()) {
        menu->addAction(CronCoinUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(CronCoinUnit newUnits)
{
    setText(CronCoinUnits::longName(newUnits));
}
