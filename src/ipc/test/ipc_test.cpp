...
void IpcSocketTest(const fs::path& datadir)
{
    ...
    BOOST_CHECK_THROW(process->bind(datadir, "test_croncoin", invalid_bind), std::invalid_argument);
    BOOST_CHECK_THROW(process->connect(datadir, "test_croncoin", invalid_bind), std::invalid_argument);

    auto bind_and_listen{[&](const std::string& bind_address) {
        std::string address{bind_address};
        int serve_fd = process->bind(datadir, "test_croncoin", address);
    ...
    auto connect_and_test{[&](const std::string& connect_address) {
        ...
        int connect_fd{process->connect(datadir, "test_croncoin", address)};
    ...
    //   Address 'unix' path '"/tmp/test_common_CronCoin Core/ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff/test_croncoin.sock"' exceeded maximum socket path length
    std::vector<std::string> addresses{
        strprintf("unix:%s", TempPath("croncoin_sock0_XXXXXX")),
        strprintf("unix:%s", TempPath("croncoin_sock1_XXXXXX")),
    };
