...
Connection::~Connection()
{
    ...
    // object, which can run arbitrary blocking croncoin code so they have to run
    ...
    // functions run, so client objects are fully disconnected before croncoin
    // code in the destructors are run. This way if the croncoin code tries to
...
void EventLoop::addAsyncCleanup(std::function<void()> fn)
{
    ...
    //
    // In croncoin core, running these callbacks in the right order is
