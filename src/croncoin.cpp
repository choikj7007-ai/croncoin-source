...
Options:
  -m, --multiprocess     Run multiprocess binaries croncoin-node, croncoin-gui.
  -M, --monolithic       Run monolithic binaries croncoind, croncoin-qt. (Default behavior)
  -v, --version          Show version information
  -h, --help             Show full help message

Commands:
  gui [ARGS]     Start GUI, equivalent to running 'croncoin-qt [ARGS]' or 'croncoin-gui [ARGS]'.
  node [ARGS]    Start node, equivalent to running 'croncoind [ARGS]' or 'croncoin-node [ARGS]'.
  rpc [ARGS]     Call RPC method, equivalent to running 'croncoin-cli -named [ARGS]'.
  wallet [ARGS]  Call wallet command, equivalent to running 'croncoin-wallet [ARGS]'.
  tx [ARGS]      Manipulate hex-encoded transactions, equivalent to running 'croncoin-tx [ARGS]'.
  help           Show full help message.
)";

static constexpr auto HELP_FULL = R"(
Additional less commonly used commands:
  bench [ARGS]      Run bench command, equivalent to running 'bench_croncoin [ARGS]'.
  chainstate [ARGS] Run croncoin kernel chainstate util, equivalent to running 'croncoin-chainstate [ARGS]'.
  test [ARGS]       Run unit tests, equivalent to running 'test_croncoin [ARGS]'.
  test-gui [ARGS]   Run GUI unit tests, equivalent to running 'test_croncoin-qt [ARGS]'.
)";
...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        if (cmd.show_help || cmd.command.empty()) {
            ...
            args.emplace_back(UseMultiprocess(cmd) ? "croncoin-gui" : "croncoin-qt");
        } else if (cmd.command == "node") {
            args.emplace_back(UseMultiprocess(cmd) ? "croncoin-node" : "croncoind");
        } else if (cmd.command == "rpc") {
            args.emplace_back("croncoin-cli");
            // Since "croncoin rpc" is a new interface that doesn't need to be
            ...
            args.emplace_back("croncoin-wallet");
        } else if (cmd.command == "tx") {
            args.emplace_back("croncoin-tx");
        } else if (cmd.command == "bench") {
            args.emplace_back("bench_croncoin");
        } else if (cmd.command == "chainstate") {
            args.emplace_back("croncoin-chainstate");
        } else if (cmd.command == "test") {
            args.emplace_back("test_croncoin");
        } else if (cmd.command == "test-gui") {
            args.emplace_back("test_croncoin-qt");
        } else if (cmd.command == "util") {
            args.emplace_back("croncoin-util");
        } else {
...
//! Execute the specified croncoind, croncoin-qt or other command line in `args`
...
//!             be a relative path to a croncoind, croncoin-qt or other executable
...
//!
//!     strace -e trace=execve -s 10000 build/bin/croncoin ...
...
static void ExecCommand(const std::vector<const char*>& args, std::string_view wrapper_argv0)
{
    ...
    // (https://github.com/croncoin/croncoin/pull/31375#discussion_r1861814807)
