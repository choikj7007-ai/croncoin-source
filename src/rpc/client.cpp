...
//! Specify whether parameter should be parsed by croncoin-cli as a JSON value,
 ...
 *
 * JSON parameters need to be listed here to make croncoin-cli parse command line
 * arguments as JSON, instead of passing them as raw strings. `JSON` and
 * `JSON_OR_STRING` formats both make `croncoin-cli` attempt to parse the
 ...
 * `croncoin-cli getblockstats <hash>` without needing to quote the hash string
 * as JSON (`'"<hash>"'`).
 *
 ...
 * to make croncoin-cli treat them as positional parameters when `-named` is used.
 * This prevents `croncoin-cli` from splitting strings like "my=wallet" into a named
 ...
 * other string parameters for that method need to be listed as well so croncoin-cli
 ...
 *
 * For example, the command `croncoin-cli -named createwallet "my=wallet"`,
