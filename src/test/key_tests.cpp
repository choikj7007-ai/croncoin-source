...
BOOST_AUTO_TEST_CASE(key_test1)
{
    ...
    // Verify address encode/decode round-trip
    std::string addr1 = EncodeDestination(PKHash(pubkey1));
    std::string addr2 = EncodeDestination(PKHash(pubkey2));
    std::string addr1C = EncodeDestination(PKHash(pubkey1C));
    std::string addr2C = EncodeDestination(PKHash(pubkey2C));
