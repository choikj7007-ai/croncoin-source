...
void MinerTestingSetup::TestPackageSelection(const CScript& scriptPubKey, const std::vector<CTransactionRef>& txFirst)
{
    ...
    tx.vout.resize(1);
    tx.vout[0].nValue = 600000000LL - 1000;
    ...
    tx.vout[0].nValue = 600000000LL - 10000;
    ...
    tx.vout[0].nValue = 600000000LL - 1000 - 50000; // 50k satoshi fee
    ...
    tx.vout[0].nValue = 600000000LL - 1000 - 50000; // 0 fee
    ...
    tx.vout[0].nValue = 600000000LL - 1000 - 50000 - feeToUse;
    ...
    tx.vout.resize(2);
    tx.vout[0].nValue = 600000000LL - 100000000;
    ...
    tx.vout[0].nValue = 600000000LL - 100000000 - feeToUse;
...
void MinerTestingSetup::TestBasicMining(const CScript& scriptPubKey, const std::vector<CTransactionRef>& txFirst, int baseheight)
{
    ...
    const CAmount BLOCKSUBSIDY = 600000 * COIN;
    const CAmount LOWFEE = COIN;       // 1000 cros - must be high enough for large txs to pass blockMinFeeRate
    const CAmount HIGHFEE = 10 * COIN; // 10000 cros
    const CAmount HIGHERFEE = 40 * COIN; // 40000 cros
...

    {
        ...
        BOOST_REQUIRE(mining->createNewBlock(options));
        // Extend to a 175000-long block chain.
        while (m_node.chainman->ActiveChain().Tip()->nHeight < 175000) {
            CBlockIndex* prev = m_node.chainman->ActiveChain().Tip();
            CBlockIndex* next = new CBlockIndex();
            next->phashBlock = new uint256(m_rng.rand256());
            m_node.chainman->ActiveChainstate().CoinsTip().SetBestBlock(next->GetBlockHash());
    ...
    // Advance height by 1 and time by SEQUENCE_LOCK_TIME so that all
    // locked txs (both absolute and relative) become final and can be mined.
    // Note: with BIP68 active and TestBlockValidity enabled in createNewBlock,
    // non-BIP68-final txs cause block validation to fail, so we must advance
    // past the lock requirements before building the template.
    for (int i = 0; i < CBlockIndex::nMedianTimeSpan; ++i) {
        CBlockIndex* ancestor{Assert(m_node.chainman->ActiveChain().Tip()->GetAncestor(m_node.chainman->ActiveChain().Tip()->nHeight - i))};
...
void MinerTestingSetup::TestPrioritisedMining(const CScript& scriptPubKey, const std::vector<CTransactionRef>& txFirst)
{
    ...
    tx.vin[0].scriptSig = CScript() << OP_1;
    tx.vout.resize(1);
    tx.vout[0].nValue = 600000000LL; // 0 fee
    ...
    tx_mempool.PrioritiseTransaction(hashMediumFeeTx, -100 * COIN);

    // This tx also has a low fee, but is prioritised
    tx.vin[0].prevout.hash = hashParentTx;
    tx.vout[0].nValue = 600000000LL - 1000 - 1000; // 1000 satoshi fee
...
BOOST_AUTO_TEST_CASE(CreateNewBlock_validity)
{
    ...
    for (const auto& bi : BLOCKINFO) {
        ...
        CMutableTransaction txCoinbase(*block.vtx[0]);
        {
            ...
            block.hashMerkleRoot = BlockMerkleRoot(block);
            // Mine a valid nonce for this block
            while (!CheckProofOfWork(block.GetHash(), block.nBits, Assert(m_node.chainman)->GetParams().GetConsensus())) {
                ++block.nNonce;
            }
        }
