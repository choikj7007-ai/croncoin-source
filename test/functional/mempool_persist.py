...
By default, croncoind will dump mempool on shutdown and
...
class MempoolPersistTest(CronCoinTestFramework):
    ...
    def run_test(self):
        ...
        fees = self.nodes[0].getmempoolentry(txid=last_txid)['fees']
        assert_equal(fees['base'] + Decimal('1'), fees['modified'])
        ...
        assert_equal(entry_prioritised_before_restart['fees']['base'] + Decimal('9.999'), entry_prioritised_before_restart['fees']['modified'])
        ...
        self.log.debug("Prevent croncoind from writing mempool.dat to disk. Verify that `savemempool` fails")
