...
class ConfArgsTest(CronCoinTestFramework):
    ...
    def test_dir_config(self):
        ...
        conf_path = self.nodes[0].datadir_path / 'croncoin.conf'
    ...
    def test_negated_config(self):
        ...
        ignored_file_message = [f'Data directory "{self.nodes[0].datadir_path}" contains a "croncoin.conf" file which is explicitly ignored using -noconf.']
    ...
    def test_config_file_parser(self):
        ...
        # Check that startup fails if conf= is set in croncoin.conf or in an included conf file
        bad_conf_file_path = self.nodes[0].datadir_path / "croncoin_bad.conf"
        ...
        with open(self.nodes[0].datadir_path / 'croncoin.conf', 'a') as conf:
        ...
        main_conf_file_path = self.nodes[0].datadir_path / "croncoin_main.conf"
    ...
    def test_config_file_log(self):
        ...
        # directory by croncoind.
        env, default_datadir = util.get_temp_default_datadir(Path(self.options.tmpdir, "test_config_file_log"))
        default_datadir.mkdir(parents=True)

        # Write a croncoin.conf file in the default data directory containing a
        # datadir= line pointing at the node datadir.
        node = self.nodes[0]
        conf_text = node.croncoinconf.read_text()
        conf_path = default_datadir / "croncoin.conf"
        ...
        # (conf_path, not node.croncoinconf)
    ...
    def test_ignored_conf(self):
        self.log.info('Test error is triggered when the datadir in use contains a croncoin.conf file that would be ignored '
        ...
        node.assert_start_raises_init_error([f"-conf={temp_conf.name}"], re.escape(
            f'Error: Data directory "{node.datadir_path}" contains a "croncoin.conf" file which is ignored, because a '
        ...
        # the same croncoin.conf that would be loaded anyway does not trigger an
        # error.
        self.start_node(0, [f'-conf={node.datadir_path}/croncoin.conf'])
        self.stop_node(0)
    ...
    def test_ignored_default_conf(self):
        ...
        self.log.info('Test error is triggered when croncoin.conf in the default data directory sets another datadir '
                      'and it contains a different croncoin.conf file that would be ignored')
        ...
        # croncoin.conf that would be ignored.
        node = self.nodes[0]
        (default_datadir / "croncoin.conf").write_text(f"datadir={node.datadir_path}\n")
        ...
        node.assert_start_raises_init_error([], re.escape(
            f'Error: Data directory "{node.datadir_path}" contains a "croncoin.conf" file which is ignored, because a '
            f'different configuration file "{default_datadir}/croncoin.conf" from data directory "{default_datadir}" '
    ...
    def test_acceptstalefeeestimates_arg_support(self):
        ...
        conf_file = self.nodes[0].datadir_path / "croncoin.conf"
    ...
    def run_test(self):
        ...
        conf_file = default_data_dir / "croncoin.conf"
        ...
        none_existent_conf_file = default_data_dir / "none_existent_croncoin.conf"
