...
TMPDIR_PREFIX = "croncoin_func_test_"
...
class CronCoinTestMetaClass(type):
    """Metaclass for CronCoinTestFramework.

    Ensures that any attempt to register a subclass of `CronCoinTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'CronCoinTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("CronCoinTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("CronCoinTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class CronCoinTestFramework(metaclass=CronCoinTestMetaClass):
    """Base class for a croncoin test script.

    Individual croncoin test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self, test_file):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave croncoinds and test.* datadir on exit or error")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use croncoin-cli instead of RPC for all commands")
    ...
    def shutdown(self):
        ...
        rpc_logger = logging.getLogger("CronCoinRPC")
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a croncoind test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple croncoind test nodes"""
        for node in self.nodes:
    ...
    def create_outpoints(self, node, *, outputs):
        """Send funds to a given list of `{address: amount}` targets using the croncoind
    ...
    def _start_logging(self):
        ...
        # Format logs the same as croncoind's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in croncoin.conf
    ...
    def skip_if_no_croncoind_tracepoints(self):
        """Skip the running test if croncoind has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("croncoind has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_croncoind_zmq(self):
        """Skip the running test if croncoind has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("croncoind has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if croncoin-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("croncoin-wallet has not been compiled")

    def skip_if_no_croncoin_tx(self):
        """Skip the running test if croncoin-tx has not been compiled."""
        if not self.is_croncoin_tx_compiled():
            raise SkipTest("croncoin-tx has not been compiled")

    def skip_if_no_croncoin_util(self):
        """Skip the running test if croncoin-util has not been compiled."""
        if not self.is_croncoin_util_compiled():
            raise SkipTest("croncoin-util has not been compiled")

    def skip_if_no_croncoin_chainstate(self):
        """Skip the running test if croncoin-chainstate has not been compiled."""
        if not self.is_croncoin_chainstate_compiled():
            raise SkipTest("croncoin-chainstate has not been compiled")

    def skip_if_no_croncoin_bench(self):
        """Skip the running test if bench_croncoin has not been compiled."""
        if not self.is_bench_compiled():
            raise SkipTest("bench_croncoin has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if croncoin-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("croncoin-cli has not been compiled.")
    ...
    def is_bench_compiled(self):
        """Checks whether bench_croncoin was compiled."""
        return self.config["components"].getboolean("BUILD_BENCH")

    def is_cli_compiled(self):
        """Checks whether croncoin-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether croncoin-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_croncoin_tx_compiled(self):
        """Checks whether croncoin-tx was compiled."""
        return self.config["components"].getboolean("BUILD_CRONCOIN_TX")

    def is_croncoin_util_compiled(self):
        """Checks whether croncoin-util was compiled."""
        return self.config["components"].getboolean("ENABLE_CRONCOIN_UTIL")

    def is_croncoin_chainstate_compiled(self):
        """Checks whether croncoin-chainstate was compiled."""
        return self.config["components"].getboolean("ENABLE_CRONCOIN_CHAINSTATE")
