...
class Binaries:
    """Helper class to provide information about croncoin binaries
    ...
    def node_argv(self, **kwargs):
        "Return argv array that should be used to invoke croncoind"
        return self._argv("node", self.paths.croncoind, **kwargs)

    def rpc_argv(self):
        "Return argv array that should be used to invoke croncoin-cli"
        # Add -nonamed because "croncoin rpc" enables -named by default, but croncoin-cli doesn't
        return self._argv("rpc", self.paths.croncoincli) + ["-nonamed"]

    def bench_argv(self):
        "Return argv array that should be used to invoke bench_croncoin"
        return self._argv("bench", self.paths.croncoin_bench)

    def tx_argv(self):
        "Return argv array that should be used to invoke croncoin-tx"
        return self._argv("tx", self.paths.croncointx)

    def util_argv(self):
        "Return argv array that should be used to invoke croncoin-util"
        return self._argv("util", self.paths.croncoinutil)

    def wallet_argv(self):
        "Return argv array that should be used to invoke croncoin-wallet"
        return self._argv("wallet", self.paths.croncoinwallet)

    def chainstate_argv(self):
        "Return argv array that should be used to invoke croncoin-chainstate"
        return self._argv("chainstate", self.paths.croncoinchainstate)

    def _argv(self, command, bin_path, need_ipc=False):
        """Return argv array that should be used to invoke the command. It
        either uses the croncoin wrapper executable (if CRONCOIN_CMD is set or
        need_ipc is True), or the direct binary path (croncoind, etc). When
        bin_dir is set (by tests calling binaries from previous releases) it
        always uses the direct path."""
        if self.bin_dir is not None:
            return [os.path.join(self.bin_dir, os.path.basename(bin_path))]
        elif self.paths.croncoin_cmd is not None or need_ipc:
            # If the current test needs IPC functionality, use the croncoin
            # wrapper binary and append -m so it calls multiprocess binaries.
            croncoin_cmd = self.paths.croncoin_cmd or [self.paths.croncoin_bin]
            return croncoin_cmd + (["-m"] if need_ipc else []) + [command]
        else:
            return [bin_path]
...
def get_binary_paths(config):
    ...
    binaries = {
        "croncoin": "CRONCOIN_BIN",
        "croncoind": "CRONCOIND",
        "bench_croncoin": "CRONCOIN_BENCH",
        "croncoin-cli": "CRONCOINCLI",
        "croncoin-util": "CRONCOINUTIL",
        "croncoin-tx": "CRONCOINTX",
        "croncoin-chainstate": "CRONCOINCHAINSTATE",
        "croncoin-wallet": "CRONCOINWALLET",
    }
    # Set paths to croncoin core binaries allowing overrides with environment
    # variables.
    ...
    # CRONCOIN_CMD environment variable can be specified to invoke croncoin
    # wrapper binary instead of other executables.
    paths.croncoin_cmd = shlex.split(os.getenv("CRONCOIN_CMD", "")) or None
    return paths
...
def wait_until_helper_internal(predicate, *, timeout=60, lock=None, timeout_factor=1.0, check_interval=0.05):
    ...
    from `CronCoinTestFramework` or `P2PInterface` class ensures the timeout is
...
def bpf_cflags():
    return [
        ...
        "-Wno-duplicate-decl-specifier",  # https://github.com/croncoin/croncoin/issues/32322
    ]
...
def initialize_datadir(dirname, n, chain, disable_autoconnect=True):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    write_config(os.path.join(datadir, "croncoin.conf"), n=n, chain=chain, disable_autoconnect=disable_autoconnect)
...
def get_temp_default_datadir(temp_dir: pathlib.Path) -> tuple[dict, pathlib.Path]:
    ...
    if platform.system() == "Windows":
        ...
        datadir = temp_dir / "CronCoin"
    else:
        env = dict(HOME=str(temp_dir))
        if platform.system() == "Darwin":
            datadir = temp_dir / "Library/Application Support/CronCoin"
        else:
            datadir = temp_dir / ".croncoin"
    return env, datadir
...
def append_config(datadir, options):
    with open(os.path.join(datadir, "croncoin.conf"), 'a') as f:
...
def get_auth_cookie(datadir, chain):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "croncoin.conf")):
        with open(os.path.join(datadir, "croncoin.conf"), 'r') as f:
            for line in f:
