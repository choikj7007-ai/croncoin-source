...
# Copyright 2016-present The CronCoin Core developers
...
class ToolUtils(CronCoinTestFramework):
    ...
    def skip_test_if_missing_module(self):
        self.skip_if_no_croncoin_tx()
        self.skip_if_no_croncoin_util()

    def run_test(self):
        self.testcase_dir = Path(self.config["environment"]["SRCDIR"]) / "test" / "functional" / "data" / "util"
        self.bins = self.get_binaries()
        with open(self.testcase_dir / "croncoin-util-test.json") as f:
    ...
    def test_one(self, testObj):
        ...
        if testObj["exec"] == "./croncoin-util":
            execrun = self.bins.util_argv() + testObj["args"]
        elif testObj["exec"] == "./croncoin-tx":
