...
class FeeFilterTest(CronCoinTestFramework):
    ...
    def test_feefilter(self):
        ...
        txids = [miniwallet.send_self_transfer(fee_rate=Decimal('0.2'), from_node=node1)['wtxid'] for _ in range(3)]
        ...
        txids = [miniwallet.send_self_transfer(fee_rate=Decimal('0.15'), from_node=node1)['wtxid'] for _ in range(3)]
        conn.wait_for_invs_to_match(txids)
        conn.clear_invs()

        self.log.info("Test txs paying 0.1 sat/byte are no longer received by test connection")
        txids = [miniwallet.send_self_transfer(fee_rate=Decimal('0.1'), from_node=node1)['wtxid'] for _ in range(3)]
        ...
        # as well.
        txids = [miniwallet.send_self_transfer(fee_rate=Decimal('20'), from_node=node0)['wtxid'] for _ in range(1)]
        conn.wait_for_invs_to_match(txids)
        conn.clear_invs()
        ...
        txids = [miniwallet.send_self_transfer(fee_rate=Decimal('20'), from_node=node1)['wtxid'] for _ in range(3)]
        conn.wait_for_invs_to_match(txids)
        conn.clear_invs()
