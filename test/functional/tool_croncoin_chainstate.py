...
"""Test croncoin-chainstate tool functionality

Test basic block processing via croncoin-chainstate tool, including detecting
duplicates and malformed input.

Test that croncoin-chainstate can load a datadir initialized with an assumeutxo
...
class CronCoinChainstateTest(CronCoinTestFramework):
    def skip_test_if_missing_module(self):
        self.skip_if_no_croncoin_chainstate()
    ...
    def basic_test(self):
        ...
        self.log.info(f"Test croncoin-chainstate {self.get_binaries().chainstate_argv()} with datadir: {datadir}")
    ...
    def assumeutxo_test(self, dump_output_path):
        ...
        datadir = n1.chain_path
        n1.stop_node()
        self.log.info(f"Test croncoin-chainstate {self.get_binaries().chainstate_argv()} with an assumeutxo datadir: {datadir}")
...
if __name__ == "__main__":
    CronCoinChainstateTest(__file__).main()
