#!/usr/bin/env python3
# Copyright (c) 2019-present The CronCoin Core developers
...
class ImportDescriptorsTest(CronCoinTestFramework):
    ...
    def run_test(self):
        ...
        addresses += ["crnrt1qrd3n235cj2czsfmsuvqqpr3lu6lg0ju7r77fhd", "crnrt1qfqeppuvj0ww98r6qghmdkj70tv8qpcheylfdhe"] # wpkh subscripts corresponding to the above addresses
        ...
        addresses = [
            'crnrt1qtmp74ayg7p24uslctssvjm06q5phz4yr46exvj', # m/0'/0'/0
            'crnrt1q8vprchan07gzagd5e6v9wd7azyucksq247l5fe', # m/0'/0'/1
            'crnrt1qtuqdtha7zmqgcrr26n2rqxztv5y8rafjjr7jqz', # m/0'/0'/2
            'crnrt1qau64272ymawq26t90md6an0ps99qkrse8p6523', # m/0'/0'/3
            'crnrt1qsg97266hrh6cpmutqen8s4s962aryy77p8w8h3', # m/0'/0'/4
        ]
        ...
        test_address(wpriv,
                     ...
                     solvable=True,
                     ismine=True)
        txid = w0.sendtoaddress(address, 49.999)
        vout_n = find_vout_for_address(self.nodes[0], txid, address)
        self.generatetoaddress(self.nodes[0], 6, w0.getnewaddress())
        tx = wpriv.createrawtransaction([{"txid": txid, "vout": vout_n}], {w0.getnewaddress(): 49.99})
        ...
        assert_equal(addr, 'crnrt1qdt0qy5p7dzhxzmegnn4ulzhard33s2809arjqgjndx87rv5vd0fqac803g') # Derived at m/84'/0'/0'/0
        change_addr = wmulti_priv.getrawchangeaddress('bech32') # uses change 0
        assert_equal(change_addr, 'crnrt1qt9uhe3a9hnq7vajl7a094z4s3crm9ttf8zw3f5v9gr2nyd7e3lnsn4stjk') # Derived at m/84'/1'/0'/0
        ...
        assert_equal(addr, 'crnrt1qp8s25ckjl7gr6x2q3dx3tn2pytwp05upkjztk6ey857tt50r5aeqy675k2') # Derived at m/84'/0'/0'/1
        change_addr = wmulti_pub.getrawchangeaddress('bech32') # uses change 2
        assert_equal(change_addr, 'crnrt1qp6j3jw8yetefte7kw6v5pc89rkgakzy98p6gf7ayslaveaxqyjusyw3l6h') # Derived at m/84'/1'/0'/2
        ...
        txid = wmulti_priv_big.sendtoaddress(w0.getnewaddress(), 9.99)
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as thread:
            with self.nodes[0].assert_debug_log(expected_msgs=["Rescan started from block 2454c267ddbca62ff21f2d3b81e8756c274d5ec00d3a8bb246f336801c596f55... (slow variant inspecting all blocks)"], timeout=10):
