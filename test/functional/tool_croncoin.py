...
"""Test the croncoin wrapper tool."""
...
class ToolCronCoinTest(CronCoinTestFramework):
    def set_test_params(self):
        self.setup_clean_chain = True
        self.num_nodes = 1

    def skip_test_if_missing_module(self):
        # Skip test on windows because currently when `croncoin node -version` is
        # run on windows, python doesn't capture output from the child
        # `croncoind` and `croncoin-node` process started with _wexecvp, and
        # stdout/stderr are always empty. See
        # https://github.com/croncoin/croncoin/pull/33229#issuecomment-3265524908
    ...
    def set_cmd_args(self, node, args):
        """Set up node so it will be started through croncoin wrapper command with specified arguments."""
        node.args = [self.binary_paths.croncoin_bin] + args + ["node"] + self.node_options[node.index]
    ...
    def run_test(self):
        ...
        self.log.info("Ensure croncoin node command invokes croncoind by default")
        self.test_args([], [], expect_exe="croncoind")

        self.log.info("Ensure croncoin -M invokes croncoind")
        self.test_args(["-M"], [], expect_exe="croncoind")

        self.log.info("Ensure croncoin -M does not accept -ipcbind")
        ...
        if self.is_ipc_compiled():
            self.log.info("Ensure croncoin -m invokes croncoin-node")
            self.test_args(["-m"], [], expect_exe="croncoin-node")

            self.log.info("Ensure croncoin -m does accept -ipcbind")
            self.test_args(["-m"], ["-ipcbind=unix"], expect_exe="croncoin-node")

            self.log.info("Ensure croncoin accepts -ipcbind by default")
            self.test_args([], ["-ipcbind=unix"], expect_exe="croncoin-node")

            self.log.info("Ensure croncoin recognizes -ipcbind in config file")
            append_config(node.datadir_path, ["ipcbind=unix"])
            self.test_args([], [], expect_exe="croncoin-node")
...
if __name__ == '__main__':
    ToolCronCoinTest(__file__).main()
