...
class InvalidBlockRequestTest(CronCoinTestFramework):
    ...
    def run_test(self):
        ...
        tx1 = create_tx_with_script(block1.vtx[0], 0, script_sig=bytes([OP_TRUE]), amount=600000 * COIN)
        tx2 = create_tx_with_script(tx1, 0, script_sig=bytes([OP_TRUE]), amount=600000 * COIN)
        ...
        block3 = create_block(tip, create_coinbase(height, nValue=600001), block_time)
        block_time += 1
        block3.solve()
        ...
        tx3 = create_tx_with_script(tx2, 0, script_sig=bytes([OP_TRUE]), amount=600000 * COIN)
