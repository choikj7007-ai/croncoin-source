...
class WalletGroupTest(CronCoinTestFramework):
    def set_test_params(self):
        ...
        for args in self.extra_args:
            args.append(f"-paytxfee=0.1")  # apply a fee rate where grouping has a cost
            args.append("-maxtxfee=10")  # CronCoin: COIN=1000 makes fees large relative to amounts
    ...
    def run_test(self):
        ...
        v.sort()
        assert_approx(v[0], vexp=0.2, vspan=0.05)
        assert_approx(v[1], vexp=0.3, vspan=0.05)
        ...
        v.sort()
        assert_approx(v[0], vexp=0.2, vspan=0.05)
        assert_approx(v[1], vexp=1.3, vspan=0.05)
        ...
        # - C1 0.5      - F  ~1.3
        # - D ~0.3
        assert_approx(self.nodes[1].getbalance(), vexp=4.3, vspan=0.05)
        assert_approx(self.nodes[2].getbalance(), vexp=4.3, vspan=0.05)
        ...
        values.sort()
        assert_approx(values[0], vexp=0.1, vspan=0.05)
        assert_approx(values[1], vexp=1.4, vspan=0.05)
        ...
        tx4_ungrouped_fee = 15
        tx4_grouped_fee = 21
        tx5_6_ungrouped_fee = 28
        tx5_6_grouped_fee = 42
