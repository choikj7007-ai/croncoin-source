Cron Coin Core version 0.19.0.1 is now available from:

  <https://croncoincore.org/bin/croncoin-core-0.19.0.1/>
...
Cron Coin Core is supported and extensively tested on operating systems using
the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not recommended
to use Cron Coin Core on unsupported systems.
...
to open the Cron Coin Core .dmg. This is due to new signing requirements
imposed by Apple, which the Cron Coin Core project does not yet adhere too.
...
- [Reduce memory](https://github.com/croncoin/croncoin/blob/master/doc/reduce-memory.md)
  suggests configuration tweaks for running Cron Coin Core on systems with
...
- `-blockfilterindex` enables the creation of BIP158 block filters for
  ...
  currently use about 4 GiB of space.  Note: this version of CronCoin
...
- `whitebind` and `whitelist` now accept a list of permissions to
  ...
  releases.  See the `croncoind -help` output for these two options for
...
- In 0.18.0, a `./configure` flag was introduced to allow disabling BIP70 support in the GUI (support was enabled by default). In 0.19.0, this flag is now __disabled__ by default. If you want to compile Cron Coin Core with BIP70 support in the GUI, you can pass `--enable-bip70` to `./configure`. (#15584)
...
- `generate` is now removed after being deprecated in Cron Coin Core 0.18.
...
- BIP 61 reject messages were deprecated in v0.18. They are now disabled
  ...
  future version of Cron Coin Core. (#14054)

- To eliminate well-known denial-of-service vectors in Cron Coin Core,
  ...
  This prevents Cron Coin Core from sending the BIP111 NODE_BLOOM service
  ...
  of Cron Coin Core and nodes that have manually enabled BIP37 support,
...
- By default, Cron Coin Core will now make two additional outbound connections that are exclusively used for block-relay.  No transactions or addr messages will be processed on these connections. These connections are designed to add little additional memory or bandwidth resource requirements but should make some partitioning attacks more difficult to carry out. (#15759)

Miscellaneous CLI Changes
-------------------------

- The `testnet` field in `croncoin-cli -getinfo` has been renamed to
...
- `getblockchaininfo` no longer returns a `bip9_softforks` object.
  ...
  an additional `type` field describes how Cron Coin Core determines
...
- When fetching a transaction announced by multiple peers, previous versions of
  Cron Coin Core would sequentially attempt to download the transaction from each
...
- If a Tor hidden service is being used, Cron Coin Core will be bound to
...
- Transactions with outputs paying v1 to v16 witness versions (future
  ...
  and services can accept any valid bech32 CronCoin address without
...
- A new wallet flag `avoid_reuse` has been added (default off). When
  ...
  partial spends" (added in Cron Coin Core v0.17.0), this can eliminate a
...
- #15649 Add ChaCha20Poly1305@CronCoin AEAD (jonasschnelli)
...
- #15639 croncoin-wallet tool: Drop libcroncoin_server.a dependency (ryanofsky)
...
- #16366 init: Use InitError for all errors in croncoind/qt (MarcoFalke)
...
- #15239 scripts and tools: Move non-linux build source tarballs to "croncoin-binaries/version" directory (hebasto)
...
- #16258 [MSVC]: Create the config.ini as part of croncoind build (NicolasDorier)
...
- #16441 remove qt libjpeg check from croncoin_qt.m4 (fanquake)
...
- #16750 msbuild: adds bench_croncoin to auto generated project files (sipsorcery)
...
- #15773 Add CronCoinTestFramework::sync_* methods (MarcoFalke)
...
- #15352 Reduce noise level in test_croncoin output (practicalswift)
...
- #15896 feature_filelock, interface_croncoin_cli: Use PACKAGE_NAME in messages rather than hardcoding Cron Coin Core (luke-jr)
...
- #16277 Suppress output in test_croncoin for expected errors (gertjaap)
...
- #16489 log: harmonize croncoind logging (jonatack)
...
- #17449 fix uninitialized variable nMinerConfirmationWindow (croncoinVBR)

### Documentation
- #15514 Update Transifex links (fanquake)
- #15513 add "sections" info to example croncoin.conf (fanquake)
...
- #15683 Comment for seemingly duplicate LIBCRONCOIN_SERVER (Bushstar)
...
- #16080 build/doc: update croncoin_config.h packages, release process (jonatack)
...
- #16448 add note on precedence of options in croncoin.conf (fanquake)
...
- #16621 add default croncoin.conf locations (GChuf)
...
- #16863 Add a missing closing parenthesis in the croncoin-wallet's help (darosior)
- #16757 CChainState return values (MarcoFalke)
- #16847 add comments clarifying how local services are advertised (jamesob)
- #16812 Fix whitespace errs in .md files, croncoin.conf, and Info.plist.in (ch4ot1c)
...
- #16912 Remove Doxygen intro from src/croncoind.cpp (ch4ot1c)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/croncoin/croncoin/).
