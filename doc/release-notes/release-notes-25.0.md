...
Cron Coin Core version 25.0 is now available from:

  <https://croncoincore.org/bin/croncoin-core-25.0/>
...
- All JSON-RPC methods accept a new [named
  parameter](https://github.com/croncoin/croncoin/blob/master/doc/JSON-RPC-interface.md#parameter-passing) called `args` that can
  contain positional parameter values. This is a convenience to allow some
  parameter values to be passed by name without having to name every value. The
  python test framework and `croncoin-cli` tool both take advantage of this, so
  for example:

```sh
croncoin-cli -named createwallet wallet_name=mywallet load_on_startup=1
```

Can now be shortened to:

```sh
croncoin-cli -named createwallet mywallet load_on_startup=1
```
...
dbcache, Cron Coin Core will now return an error at startup. (#25574)
...
before Cron Coin Core has begun its shutdown sequence. (#23395)
...
- In the `createwallet`, `loadwallet`, `unloadwallet`, and `restorewallet` RPCs, the
  ...
  deprecation period by launching croncoind with the configuration option
...
- The binary verification script has been updated. In previous releases it
  ...
  examples, see:
  https://github.com/croncoin/croncoin/blob/master/contrib/verify-binaries/README.md
  (#27358)
