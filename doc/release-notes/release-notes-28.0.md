Cron Coin Core version 28.0 is now available from:

  <https://croncoincore.org/bin/croncoin-core-28.0/>
...
Running Cron Coin Core binaries on macOS requires self signing.
```
cd /path/to/croncoin-28.0/bin
xattr -d com.apple.quarantine croncoin-cli croncoin-qt croncoin-tx croncoin-util croncoin-wallet croncoind test_croncoin
codesign -s - croncoin-cli croncoin-qt croncoin-tx croncoin-util croncoin-wallet croncoind test_croncoin
```
...
Support for Testnet4 as specified in [BIP94](https://github.com/croncoin/bips/blob/master/bip-0094.mediawiki)
...
The default data directory on Windows has been moved from `C:\Users\Username\AppData\Roaming\CronCoin`
to `C:\Users\Username\AppData\Local\CronCoin`. Cron Coin Core will check the existence
...
See [JSON-RPC-interface.md](https://github.com/croncoin/croncoin/blob/master/doc/JSON-RPC-interface.md#json-rpc-11-vs-20) for details. (#27101)
...
libcroncoinconsensus Removal
---------------------------

The libcroncoin-consensus library was deprecated in 27.0 and is now completely removed. (#29648)

P2P and Network Changes
-----------------------

- Previously if Cron Coin Core was listening for P2P connections, either using
...
- Cron Coin Core will now fail to start up if any of its P2P binds fail, rather
...
- Transactions with version number set to 3 are now treated as standard on all networks (#29496),
  ...
  described in [BIP 431](https://github.com/croncoin/bips/blob/master/bip-0431.mediawiki).  The
...
- The `warnings` field in `getblockchaininfo`, `getmininginfo` and
  ...
  can be temporarily restored by running Cron Coin Core with the configuration
...
are now required to compile Cron Coin Core. (#29091, #30263)

- The minimum required glibc to run Cron Coin Core is now
...
- A new `-testdatadir` option has been added to `test_croncoin` to allow specifying the
...
Previous releases of Cron Coin Core or previous external software will not be able to read the blocksdir with a non-zero XOR-key.
