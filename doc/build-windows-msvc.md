...
This guide describes how to build croncoind, command-line utilities, and GUI on Windows using Microsoft Visual Studio.
...
### 3. Clone CronCoin Repository

Clone the Cron Coin Core repository to a directory. All build scripts and commands will run from this directory.
```
...
The Cron Coin Core project supports the following vcpkg triplets:
- `x64-windows` (both CRT and library linkage is dynamic)
- `x64-windows-static` (both CRT and library linkage is static)

To facilitate build process, the Cron Coin Core project provides presets, which are used in this guide.
...
vcpkg installation during the configuration step might fail for various reasons unrelated to Cron Coin Core.
...
can occur if your local Cron Coin Core repository path contains spaces, you can override the vcpkg install directory
...
To improve the build process performance, one might add the CronCoin repository directory to the Microsoft Defender Antivirus exclusions.
