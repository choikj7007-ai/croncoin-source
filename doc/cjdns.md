# CJDNS support in Cron Coin Core

It is possible to run Cron Coin Core over CJDNS, an encrypted IPv6 network that
...
done outside of Cron Coin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the CronCoin network and individual nodes.
...
before it will work with your Cron Coin Core node. This is described in steps
...
## Run Cron Coin Core with CJDNS

Once you are connected to the CJDNS network, the following Cron Coin Core
...
When enabled, this option tells Cron Coin Core that it is running in an
...
IPv6 local network. This helps Cron Coin Core perform better address management:
...
CJDNS support was added to Cron Coin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `croncoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/croncoin/croncoin/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Cron Coin Core

There are several ways to see your CJDNS address in Cron Coin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `croncoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `croncoin-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `croncoin-cli help getnodeaddresses` for details.
